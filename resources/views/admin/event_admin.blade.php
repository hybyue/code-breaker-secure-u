                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex justify-content-center">
                                <div class="mx-1">
                                    <a href="#" class="btn btn-sm text-white" style="background-color: #063292" data-bs-toggle="modal" data-bs-target="#updateEvent-{{ $event->id }}">
                                        <i class="bi bi-pencil-square"></i>
                                    </a>
                                </div>
                                <div class="mx-1">
                                    <form action="{{route('archive.events', $event->id)}}" method="POST" onsubmit="return confirm('Are you sure you want to Archive?')">
                                        @csrf
                                        @method('DELETE')
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addEventForm" action="{{ route('event.store_admin') }}" method="POST">
                    @csrf
                    <div class="mb-3">
...
<div class="modal fade" id="updateEvent-{{ $event->id }}" tabindex="-1" aria-labelledby="updateEventModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{ route('update.events_admin', $event->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="modal-header">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="eventDate" class="form-label">Date End:</label>
                        <input type="date" class="form-control" id="eventDate" name="date_end" value="{{ \Carbon\Carbon::parse($event->date_start)->format('Y-m-d') }}" required>
                    </div>
                </div>
